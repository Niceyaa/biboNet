...
<script>
					...
					success: function(loginRes) {
						...
						uni.getUserInfo({
							...
							success: function(res) {
								console.log('getuserinfo', res);
								uni.setStorageSync('city', res.userInfo.city);	
						loginApi({
							appid: that.appid,
							code: loginRes.code,
							...
							city_name: e.detail.userInfo.city
						}).then(function(data) {
							// 保存 sign
							console.log('小程序登录', data);
								uni.showToast({
									title: '执行到loginApi'
								});
								if (data[1].data.err_code === 302) {
	...
	>>>>>>> b1d161b82c9e7bb267ce3720ba99fe496139c8ff
								}
							});
							}
						}
